use std::{os, result, str::from_utf8};
...
impl<E: StorageEngine + Send + Sync> NaadanServer<E> {
    ...
    pub async fn start(self: Self) {
        ...
        loop {
            ...
            utils::log(
                "Server".to_string(),
                format!("Got new request from IP: {}", socket.peer_addr().unwrap()),
            );
        ...
        loop {
...

            {
                ...
                for query_result in query_results {
                    match query_result {
                        Ok(val) => {
                            let mut slice = val.to_string();
                            slice.push_str("\n");

                            result.append(&mut slice.as_bytes().to_vec());
                        }
